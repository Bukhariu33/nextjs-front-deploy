describe(`should test fund managers support flow`, () => {
  beforeEach(() => {
    cy.visit('auth/sign-in');
    cy.loginAndVisit('fundManager', '/fund-manager/support');
  });

  it('should create a new support ticket', () => {
    cy.get('[data-cy-button="createNewTicket"]').click();
    ...
    cy.wait('@createRequest').then(({ response }) => {
      ...
      cy.url().should(
        'contain',
        `/fund-manager/support/${response?.body?.data?.id}`,
      );
    });
